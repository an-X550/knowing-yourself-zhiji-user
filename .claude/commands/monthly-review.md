...
从 `## 六、下月规划` 只提取下月可检查的阶段结果，并显示最多 3 项 `SMART 标题｜绝对截止日期或时间`。没有候选时只输出“本次没有可直接创建的滴答任务。”，不显示空任务块。用户可用自然语言修改或删除；修改后必须重新展示一次最终整组。只有用户对最终整组明确确认后才创建；生成请求中预先说“同步到滴答”不算最终确认，含糊表达不创建。未确认候选不排队、不催办、不写入状态。
